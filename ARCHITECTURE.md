...
A Bukkit server plugin (Java 8 bytecode, running on CraftBukkit, Spigot, Paper or a fork) that watches server events and posts them to a Discord channel over a webhook, either as rich embeds or plain Markdown text. It ships two versioned files — `config.yml` (what to log, where, and how it looks) and `lang.yml` (every string it says) — both of which migrate themselves forward automatically. It checks for updates in a way that's aware of which release channel it's running on, and is accompanied by a Jekyll website (this `docs/` folder) with an interactive config generator.
...
Migration is **forward-only and sequential** — v8 to v11 goes through v9 and v10, never straight across. Most of the work is generic: parse both files, transplant the user's values into the new structure, preserve every comment and inline spacing. But schemas that *moved* keys need real code, and there is a small `step()` chain for exactly those (`v6ToV7`, `v9ToV10`); v10 to v11 only added keys, so it needs no code at all; every other step is the identity function. A config from a *newer* build is never downgraded — the plugin says so and changes nothing. Both managed files go through this, and each keeps its own backup (`config.old.yml`, `lang.old.yml`).
...
**`Vanish` and `Placeholders`** both reach other plugins without depending on them, and for the same reason: roughly a third to two-thirds of servers don't run the plugin in question, and none of them should carry a compile-time coupling to it. `Vanish` reads the `vanished` metadata value that EssentialsX, SuperVanish, PremiumVanish and CMI all set and all document — one read covering every one of them, including ones nobody here has heard of. `Placeholders` resolves PlaceholderAPI's entry point reflectively, once, and caches it. Both are inert when the plugin is absent, which is why call sites use them unconditionally.

**`CustomLogs`** is how a server logs plugins DiscordLogger has never heard of. An admin declares a rule under `log.custom.<name>` — a command to watch, a message, a colour — and gets a real event with its own routing. It reuses everything: `Log.init` already walks `log.<group>.<event>` for colour and webhook, so this needed no new machinery at all. The honest limit is that it sees a command *run*, not an action *succeed*, which is exactly the distinction the moderation listeners exist to make.

**`OpAlert`** tells staff in game when logging has broken. That sounds like a nicety and isn't: eleven days of metrics showed 7,957 of 8,721 send failures were deleted webhooks, spread over 130 separate windows, with nobody noticing — because the only warning went to console. It fires for the two failures an admin can act on, is capped at one message per problem per 30 minutes, and never prints a URL.
...
2. **`docs/assets/configs/v<N>/config.yml`** — a static, byte-for-byte mirror served by the plain "Download" button on the docs site, for people who don't want to use the wizard.
3. **`docs/assets/configs/v<N>/config.template.yml`** — the same shape, but with `{{TOKEN}}` placeholders instead of real values. The generator wizard fills these in based on what a visitor chooses; this file itself is never downloaded.
4. **A code block embedded directly in `docs/config/v<N>/index.md`** — the full file shown inline for people reading the documentation who don't want to click away.
