---
layout: default
title: "config.yml v11 Reference — Every Key Explained"
description: Full documentation for DiscordLogger's config.yml and lang.yml at schema v11 — every key explained, with downloadable files.
---

# config.yml Docs — v11

**_Supported Plugin Versions:_ <span data-dl-schema-versions="v11">…</span>**

<div style="margin:1rem 0 .5rem;">
  <a class="btn" href="/assets/configs/v11/config.yml" download>
    Download v11 config.yml
  </a>
</div>

<div style="margin:0 0 1.25rem;">
  <a class="btn" href="/assets/configs/v11/lang.yml" download>
    Download v11 lang.yml
  </a>
</div>
...
Works with **EssentialsX**, **SuperVanish**, **PremiumVanish** and **CMI**, with nothing
to install and no setup — all of them mark a hidden player the same way, and this reads
that mark. It also covers vanish plugins not listed here, provided they follow the same
convention.
...
While vanished, every **player** event stays quiet — join, quit, chat, death,
advancement, teleport and gamemode.

> **Moderation is never affected.** A ban, kick or whitelist change is logged whether or
> not the staff member was vanished. Hiding staff *actions* because the person was hidden
> would gut the audit trail exactly when it matters most — the same reasoning that keeps
> `ignored_players` from suppressing moderation.
...
filters:
  ...
  # Only does anything on Minecraft 1.12 or newer, where advancements exist.
  ignored_advancements: []
  ...
  # Some of these arrived in later Minecraft versions -- CHORUS_FRUIT and
  # END_GATEWAY in 1.9, CONSUMABLE_EFFECT much later. Naming one your server
  # does not have is harmless: it never fires, so the entry simply does nothing.
  ignored_teleport_causes:
    - EXIT_BED      # standing up from a bed
    - DISMOUNT      # getting off a horse, boat or minecart
    - SPECTATE      # a spectator jumping to a player
  ...
  # Newer causes only exist on newer servers -- FREEZE from 1.17, SONIC_BOOM from
  # 1.19. Listing one your server does not have is harmless; it never fires.
  ignored_death_causes: []
  ...
  # The block ones need Minecraft 1.8.3 or newer, and RESPAWN_ANCHOR 1.16 -- below
  # that there is no block explosion to filter, so those entries do nothing.
  ignored_explosion_sources: []
...
log:
  player:
    ...
    advancement: # Logs when a player gets an advancement
      ...
      # achievements. This one switch covers both: on a server old enough to have
      # achievements instead, they are reported here too rather than under a
      # separate setting, since they mean the same thing to a reader.
      # (Achievement support is not built yet -- on 1.8 to 1.11 this logs nothing.)
      enabled: true
      color: "#2ECC71" # green
      webhook: "" # Send just this event elsewhere. Empty = use webhook.url above
