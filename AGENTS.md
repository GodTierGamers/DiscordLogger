...
| `<spigot.api.version>` | `1.13-R0.1-SNAPSHOT` | what the normal build and the **test suite** compile against. Spigot, not Paper: Paper implements the Bukkit API, so compiling against the smaller of the two gives up nothing and one JAR serves both. Deliberately the same number as the floor below |
...
**The floors used to be three different numbers. They are now two, and that is the goal state.** `api-version: 1.13` admits 1.13.0 onward, because api-version cannot express a patch before 1.20.5. `<dl.compat.floor>` is 1.13.0, and `<spigot.api.version>` is now *also* 1.13.0 — the build sits on the floor rather than above it. The gap between those two existed only because the test suite could not run at the floor: Bukkit below 1.19.4 wanted SnakeYAML 1.x and collided with our shaded 2.x on the unrelocated test classpath. SnakeYAML is the server's now rather than shaded, so the reason is gone, and building above the floor would only let in API that a floor-version server does not have — 1.13.1 and 1.13.2 each added some.

**The compat-floor job stays even though it now passes trivially.** It is what fails if the two ever drift apart again: raise the build to a newer API without raising the floor, and the job rebuilds at the floor and catches the call that a floor-version server could not satisfy. Cheap insurance against exactly the mistake that is easy to make while lowering support further.
