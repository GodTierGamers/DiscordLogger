...
The sync script derives three values no property owns: **`plugin`** (the released version, from `<version>`), **`schema`** (the config schema, read from `config.yml`'s trailer), and **`paper_display`** (the supported range in prose, e.g. `1.8 – 26.2`, built from the first and last of `<dl.game.versions>`; the key is still named for Paper from when Paper was required, which is now misleading — renaming it touches the sync script and every template that reads it, so it is worth doing deliberately rather than in passing). Docs examples of the config trailer use these, so they can't go stale when a release ships or the schema moves. `paper_display` is derived rather than hand-set because a standalone property would drift the moment a Minecraft version is added — the listings would advertise the new ceiling while every badge and requirements table still named the old one. (A `<dl.paper.display>` property did exist and has been removed; if you see it referenced, that's stale.)
