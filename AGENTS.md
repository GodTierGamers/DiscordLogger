...
| `<dl.compat.floor>` | `1.8.8-R0.1-SNAPSHOT` | the oldest server the plugin promises to **run on**; CI's `compat-floor` job compiles against it with the two gated files excluded |
| `<spigot.api.version>` | `1.13-R0.1-SNAPSHOT` | what the normal build and the **test suite** compile against. Spigot, not Paper: Paper implements the Bukkit API, so compiling against the smaller of the two gives up nothing and one JAR serves both. Higher than the floor, because two listeners name events that arrived after it |
...
**The build version and the floor are deliberately different numbers, and the gap is the whole design.** `<dl.compat.floor>` is 1.8.8 — the oldest server the plugin runs on. `<spigot.api.version>` is 1.13.0, because two listeners name events that arrived after the floor: `PlayerAdvancementDoneEvent` (1.12, and advancements did not exist before it — 1.8 had achievements, a different event 1.12 removed) and `PlayerCommandSendEvent` (1.13).

**Those two are gated at construction, not inside their handlers.** `util.Compat` looks them up by name and returns `null` when the event is absent, so the classes themselves stay ordinary compiled code with full type checking and working unit tests. Probing the API *inside* each handler would have put the feature logic somewhere no compiler and no test can reach — and advancement logging is a feature every current server uses, where silent breakage reads as "advancements stopped appearing" and nobody files a bug.

**The `compat-floor` profile is what keeps this honest.** It compiles at 1.8.8 with exactly those two files excluded, so every other file is machine-checked against the floor. Run without the profile it fails with twelve errors, which is the proof the exclusion list is load-bearing rather than decorative. `CompatTest` covers the other half: the class names and constructor signatures the gates use are declared once on `Compat` and asserted against the real classes, so a rename fails the build instead of quietly unregistering a listener.
...
| `compat-floor` | same | recompiles with `-Pcompat-floor -Dspigot.api.version=<dl.compat.floor>`, so using an API added after the floor fails here instead of `NoSuchMethodError`-ing on a user's server. The profile excludes the two gated files; **everything else must compile at 1.8.8** |
