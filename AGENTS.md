...
| `<dl.game.versions>` | 74 entries, `1.8`–`26.2` | the supported range; the listings advertise it, and the prose + badge in README/CONTRIBUTING are derived from its first and last entries |

The sync script derives three values no property owns: **`plugin`** (the released version, from `<version>`), **`schema`** (the config schema, read from `config.yml`'s trailer), and **`mc_display`** (the supported range in prose, e.g. `1.8 – 26.2`, built from the first and last of `<dl.game.versions>`). The data keys are named for Minecraft and the API, not for Paper: `mc_display`, `mc_badge`, `api_version` and `api_artifact`. They were `paper_*` and `min_paper` while Paper was a requirement, and were renamed once it stopped being one — a key called `min_paper` on a plugin that runs on CraftBukkit is a comment that lies. Docs examples of the config trailer use these, so they can't go stale when a release ships or the schema moves. `mc_display` is derived rather than hand-set because a standalone property would drift the moment a Minecraft version is added — the listings would advertise the new ceiling while every badge and requirements table still named the old one. (A `<dl.paper.display>` property did exist and has been removed; if you see it referenced, that's stale.)
...
**The build version and the floor are deliberately different numbers, and the gap is the whole design.** `<dl.compat.floor>` is 1.8.0 — the oldest server the plugin runs on, and the lowest `spigot-api` published anywhere, so it is the floor in the literal sense rather than a chosen one. `<spigot.api.version>` is 1.13.0, because three listeners name events that arrived after the floor: `BlockExplodeEvent` (1.8.3), `PlayerAdvancementDoneEvent` (1.12, and advancements did not exist before it — 1.8 had achievements, a different event 1.12 removed) and `PlayerCommandSendEvent` (1.13).

**`BlockExplodeEvent` is why `BlockExplosion` is its own class.** It sits beside `Explosion`, which handles entity explosions and works fine on 1.8.0. Had both handlers stayed in one class, the missing type would have made the class unloadable and taken creeper and TNT logging down with it on the very servers the split exists to serve; the two share `Explosion`'s helpers so the embeds stay identical.

**All three are gated at construction, not inside their handlers.** `util.Compat` looks them up by name and returns `null` when the event is absent, so the classes themselves stay ordinary compiled code with full type checking and working unit tests. Probing the API *inside* each handler would have put the feature logic somewhere no compiler and no test can reach — and advancement logging is a feature every current server uses, where silent breakage reads as "advancements stopped appearing" and nobody files a bug.

**Adventure is the one dependency the floor job cannot vouch for.** `adventure-platform-bukkit` supports 1.8.8 and up, and the floor is 1.8.0, so `util.Chat` catches a failed `BukkitAudiences.create` and falls back to legacy section codes. Players on 1.8.0–1.8.7 still get coloured, readable messages; what legacy cannot carry is hover, click and true hex, none of which a client that old renders anyway. bStats needed no such handling — every Bukkit call it makes resolves at 1.8.0, `getOnlinePlayers()` included, which was worth checking because the floor job only ever looks at our own source and never at a shaded library.

**The `compat-floor` profile is what keeps this honest.** It compiles at 1.8.0 with exactly those three files excluded, so every other file is machine-checked against the floor. Run without the profile it fails with sixteen errors, which is the proof the exclusion list is load-bearing rather than decorative. `CompatTest` covers the other half: the class names and constructor signatures the gates use are declared once on `Compat` and asserted against the real classes, so a rename fails the build instead of quietly unregistering a listener.

**A separate number, `<dl.game.versions>`, is editorial and is not derived from any of them.** It is what the Modrinth, Hangar and CurseForge listings advertise, and it now reads 1.8–26.2 — widened once the acceptance suite had actually driven every setting on 1.8, 1.8.8, 1.12.2, 1.16.5 and 26.2, rather than as a side effect of lowering the compile floor. Supporting more than you advertise is the safe direction; advertising more than you have tested is not. Hangar is published a shorter list, because its PAPER platform means "runs on Paper" and Paper has no build below 1.8.8.
...
- **Java 8 bytecode, Bukkit API only.** 8 is the compile target because servers from 1.8 to 1.16 run on it; do not reach for a newer language feature without moving `<maven.compiler.release>`, which raises the floor for every server on the list. `String.isBlank`, `List.of` and friends are Java 9+ and will compile on your machine and fail in CI — `Strings.isBlank` exists for exactly that reason. Nothing Paper-only: the plugin runs on CraftBukkit, and `compat-floor` fails the build if that stops being true. Adventure preferred for anything new touching chat components (`ChatColor` lingers in command feedback).
