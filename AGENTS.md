...
2. **Conventional Commit PR titles** (`feat:` / `fix:` / `docs:` / `chore:` / `refactor:` / `ci:` / `test:` …) — `lint-pr.yml` rejects anything else. The title becomes the changelog entry verbatim. Squash-merge.
   ...
   **Pick the type by what changes in the JAR, not by how much work it was.** `feat:` and `fix:` move the version, so they are claims about the *shipped artifact*. Something under `scripts/`, `.github/` or `docs/` that never reaches a user's server is `chore:`, `ci:` or `docs:` however substantial it is — a new maintainer tool is not a feature of the plugin. `scripts/export-metrics.py` went in as `feat:`, which turned an otherwise patch-only cycle into a 2.4.0 minor release for a file no server ever loads; it was retargeted with `Release-As:`. Getting this wrong is only fixable after the fact, so weigh it when writing the title.
