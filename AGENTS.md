...
**Superseded schemas get a legacy banner automatically.** `applyLegacyNotice` in `versions.js` reads the page's schema from the URL (`/config/v10/`) and injects a notice when a newer schema exists *and* a published release carries it. Derived from the URL rather than written into each page for two reasons: publication freezes a schema's files, so a banner in `docs/config/v9/index.md` would mean editing what must not change; and deriving it means every future version gains the notice with nothing to remember. A schema merely declared in the registry does not make its predecessor legacy — until the release exists, the older page is still the current one.
